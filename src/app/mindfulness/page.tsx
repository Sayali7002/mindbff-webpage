...
export default function MindfulnessPage() {
  ...
  const [streakVersion, setStreakVersion] = useState<number>(0);
 ...
 {/*}   const savedStreak = localStorage.getItem('mindfulness_streak');
  ...
  useEffect(() => {
    const fetchStreakData = async () => {
      if (user) {
        try {
          // Check if we have cached streak data
          const cachedStreakData = localStorage.getItem('cachedStreakData');
          const cachedVersion = localStorage.getItem('cachedStreakVersion');
          
          if (cachedStreakData && cachedVersion) {
            // Parse the cached data
            const parsedData = JSON.parse(cachedStreakData);
            const parsedVersion = parseInt(cachedVersion);
            
            // Set the cached data in state
            setStreakData(parsedData);
            setStreakVersion(parsedVersion);
            
            // Make a lightweight HEAD request to check if the streak has been updated
            ...
            // Use a HEAD request to check if the streak has been updated
            const response = await fetch('/api/mindfulness/streak?version=' + parsedVersion, {
              method: 'HEAD',
              headers: {
            ...
            // If the response indicates the streak has been updated (status 200), fetch the new data
            if (response.status === 200) {
              const result = await fetch('/api/mindfulness/streak', {
                headers: {
                  'Authorization': `Bearer ${session.access_token}`,
                },
              }).then(res => res.json());
              ...
              if (result.data) {
                ...
                setStreakVersion(newVersion);
                localStorage.setItem('cachedStreakData', JSON.stringify(result.data));
                localStorage.setItem('cachedStreakVersion', newVersion.toString());
              }
            }
          } else {
            ...
            if (result.data) {
              ...
              setStreakVersion(1);
              localStorage.setItem('cachedStreakData', JSON.stringify(result.data));
              localStorage.setItem('cachedStreakVersion', '1');
            }
          }
        } catch (error) {
        ...
        setStreakVersion(0);
        localStorage.removeItem('cachedStreakData');
        localStorage.removeItem('cachedStreakVersion');
      }
    };
  ...
  const updateStreak = async (activityType: 'mindfulness') => {
    ...
    try {
      ...
      if (data.data) {
        ...
        // Update the cached data and increment the version
        const newVersion = streakVersion + 1;
        setStreakVersion(newVersion);
        localStorage.setItem('cachedStreakData', JSON.stringify(data.data));
        localStorage.setItem('cachedStreakVersion', newVersion.toString());
      }
