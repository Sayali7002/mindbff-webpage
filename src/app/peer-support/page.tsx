...
export default function PeerSupportPage() {
  ...
  }, [userProfile, selectedJourneys, fetchPeers, setIsAnonymous]);
  
  // Handle client-side filtering and sorting separately to avoid unnecessary API calls
  useEffect(() => {
    if (userProfile) {
      const filters: Record<string, any> = {};
      filters.activeOnly = activeOnlyFilter === true ? true : undefined;
  ...
  }, [activeOnlyFilter, sortBy, fetchPeers]);
  ...
  const saveJourneyPreferences = async () => {
    ...
    // This will trigger a new API call since journey preferences changed
    const filters = {
    ...
    <div className="min-h-screen gradient-bg p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {!(userProfile?.supportType === 'support-giver' && !ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) && (
          ...
          {!(userProfile?.supportType === 'support-giver' && !ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) && isPeersLoading && (
  <div className="min-h-[200px] flex items-center justify-center">
    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600" />
    <span className="ml-4 text-blue-700 font-medium">Loading peers...</span>
  </div>
)}
          {!(userProfile?.supportType === 'support-giver' && !ENABLE_PEER_FETCHING_FOR_SUPPORT_GIVERS) && !isPeersLoading && (
