...
export async function POST(request: NextRequest) {
  return withAuth(request, async (userId, request) => {
    try {
      ...
      console.log(`Insert result:`, { data: insertedMessages, error: insertError });
      
      if (insertError) {
        console.error('Error sending message:', insertError);
        return NextResponse.json(
          { error: 'Error sending message: ' + insertError.message },
          { status: 500 }
        );
      }
      ...
      if (previousMessages && previousMessages.length === 1) {
        // Only one message exists, so this is the first message
        ...
        try {
          ...
          if (receiverProfile && receiverProfile.email) {
            await sendResendEmail(
              ...
              'New Peer Chat Message',
              `You have received a new message from your peer on MindBFF. Log in to view and respond.`
            );
          }
        } catch (e) {
          console.error('Failed to send peer chat email notification:', e);
        }
      }
      ...
      const chatMessage = {
        ...
        timestamp: insertedMessages[0].created_at,
        isAnonymous: insertedMessages[0].is_anonymous,
        senderId: insertedMessages[0].sender_id,
        receiverId: insertedMessages[0].receiver_id
      };
