  ...
  return withAuth(request, async (userId, request) => {
    try {
      ...
      const response = NextResponse.json({ 
       ...
       // Send email notification to receiver (backend, after insert)
       try {
         const adminClient = createSupabaseAdminClient(
           process.env.NEXT_PUBLIC_SUPABASE_URL!,
           process.env.SUPABASE_SERVICE_ROLE_KEY!
         );
         const { data: userData, error: userError } = await adminClient.auth.admin.getUserById(receiver_id);
         const receiverEmail = userData?.user?.email;
         if (receiverEmail) {
           await sendResendEmail(
             receiverEmail,
             'New Support Request',
             `You have received a new support request on MindBFF. Log in to view and respond.`
           );
         } else {
           console.error('Receiver email not found for user:', receiver_id);
         }
         if (userError) {
           console.error('Error fetching receiver email:', userError);
         }
       } catch (e) {
         console.error('Failed to send support request email notification:', e);
       }
      return response;
