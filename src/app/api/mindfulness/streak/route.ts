...
export async function GET(request: NextRequest) {
  return withAuth(request, async (userId, req) => {
    try {
      ...
      // Check if this is a version check request (HEAD request with version parameter)
      const url = new URL(req.url);
      const clientVersion = url.searchParams.get('version');
      
      if (req.method === 'HEAD' && clientVersion) {
        // This is a version check request
        ...
        if (fetchError && fetchError.code !== 'PGRST116') {
          console.error('Error fetching streak timestamp:', fetchError);
          return new NextResponse(null, { status: 500 });
        }
        
        if (!existingStreak) {
          // No streak found, client should fetch fresh data
          return new NextResponse(null, { status: 200 });
        }
        
        // Compare the last_updated timestamp with the client version
        // For simplicity, we'll just check if there's any difference
        // In a real app, you might want to use a more sophisticated versioning mechanism
        const lastUpdatedTime = new Date(existingStreak.last_updated).getTime();
        
        if (lastUpdatedTime > parseInt(clientVersion)) {
          // Data has been updated, client should fetch fresh data
          return new NextResponse(null, { status: 200 });
        } else {
          // Data hasn't changed, client can use cached data
          return new NextResponse(null, { status: 304 });
        }
      }
      
      // Regular GET request to fetch streak data
      const { data: existingStreak, error: fetchError } = await supabase
        .from('mindfulness_streaks')
        .select('*')
        .eq('user_id', userId)
