...
export default function EnhancedFocusTimer({ onClose, onComplete }: EnhancedFocusTimerProps) {
  ...
  // Handle wake lock to keep screen active during timer
  useEffect(() => {
    const requestWakeLock = async () => {
      try {
        if ('wakeLock' in navigator && isRunning && !isPaused) {
          ...
          console.log('Wake lock activated for focus timer');
        }
      } catch (err) {
    ...
    const releaseWakeLock = async () => {
      if (wakeLockRef.current) {
        try {
          await wakeLockRef.current.release();
          wakeLockRef.current = null;
          console.log('Wake lock released for focus timer');
        } catch (err) {
  ...
  useEffect(() => {
    if (isRunning && !isPaused) {
      ...
      intervalRef.current = setInterval(() => {
        ...
        setPulseAnimation((prev: boolean) => !prev);
