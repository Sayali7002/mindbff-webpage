...
export default function StrengthsTracker({ onClose }: StrengthsTrackerProps) {
  const { user } = useAuth();
  const { entries, isLoading, createStrengthEntry, deleteEntry, fetchEntries } = useMindfulness({ type: 'strength', autoFetch: true });
  ...
  // Effect to check if we need to refresh data
  useEffect(() => {
    // We'll rely on the cached data from useMindfulness hook
    // The hook will automatically use cached data if available
  }, []);
    ...
    <div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] flex flex-col" style={{ minHeight: '500px' }}>
      ...
      <div className="flex mb-4 border-b">
        ...
        <button
          ...
          onClick={() => {
            setActiveTab('past');
            // Force refresh when viewing past entries to ensure we have latest data
            fetchEntries(true);
          }}
        >
          Past Reflections
