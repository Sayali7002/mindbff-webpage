...
export default function Dashboard({ userProfile: propUserProfile, onStartProfileSetup }: DashboardProps) {
  ...
  const handleCloseStrengths = () => {
    ...
    // Refresh strengths after modal closes to ensure we have the latest data
    fetchEntries(true); // Force refresh from API
  };
                ...
                {isAuthenticated ? (
                  <div className="flex items-center">
                    ...
                    <div className="ml-2 flex items-center">
                      <div className="bg-gradient-to-r from-amber-400 to-amber-600 text-white text-xs px-2 py-1 rounded-full flex items-center">
                        <span className="material-icons text-sm mr-1">workspace_premium</span>
                        Exclusive
                      </div>
                      <Button
                        onClick={signOut}
...
<div className="grid grid-cols-2 gap-4">
    ...
    <div className="bg-[#ffe4e6] p-5 rounded-3xl shadow-sm flex flex-col">
        ...
        {/* Top concerns slider - NEW SECTION */}
        <div className="bg-white bg-opacity-80 p-4 rounded-3xl shadow-sm mb-4">
          <h2 className="text-xl font-bold text-gray-900 mb-3">Clear Your Thoughts</h2>
          <div className="overflow-x-auto pb-2">
            <div className="flex space-x-3 min-w-max">
              {[
                { concern: "Work Stress", icon: "work" },
                { concern: "Anxiety", icon: "psychology" },
                { concern: "Negative Thoughts", icon: "mood_bad" },
                { concern: "Relationship Issues", icon: "people" },
                { concern: "Sleep Problems", icon: "bedtime" },
                { concern: "Self-Doubt", icon: "help" },
                { concern: "Burnout", icon: "battery_alert" }
              ].map((item, index) => (
                <div 
                  key={index}
                  onClick={() => router.push('/dashboard/cbt-thought-tracker')}
                  className="flex-shrink-0 bg-gradient-to-r from-blue-50 to-indigo-50 p-3 rounded-xl cursor-pointer hover:shadow-md transition-all"
                >
                  <div className="flex items-center space-x-2">
                    <span className="material-icons text-indigo-600">{item.icon}</span>
                    <span className="whitespace-nowrap font-medium text-gray-800">{item.concern}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex justify-between items-center bg-[#ffe4e6] p-4 rounded-3xl shadow-sm">
          ...
          >
            <span className="material-icons text-3xl text-gray-600 mb-2">assistant</span>
